...
statement
        : compound_statement {$$.code = strdup($1.code); }
        | expression_statement {$$.code = strdup($1.code);}
        | selection_statement {$$.code = strdup($1.code); }
        | iteration_statement {$$.code = strdup($1.code); }
        | jump_statement {$$.code = strdup($1.code); }
        ;
